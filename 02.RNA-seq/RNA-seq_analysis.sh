...
python merge_salmon_result.py -folders {sample1} {sample2} {sample3}  -col NumReads -out merged_{species}_NumReads.txt
python merge_salmon_result.py -folders {sample1} {sample2} {sample3}  -col TPM -out merged_{species}_TPM.txt
python merge_salmon_result.py -folders {sample1} {sample2} {sample3}  -col EffectiveLength -out merged_{species}_EffectiveLength.txt
...
python MergeResultAcrossSpecies.py merged_{species1}_{NumReads/EffectiveLength/TPM}.txt merged_{species2}_{NumReads/EffectiveLength/TPM}.txt {species1}_{species2}_allIDs_merge.output merged_{species1}_{species2}_{NumReads/EffectiveLength/TPM}.output
